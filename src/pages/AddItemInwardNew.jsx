...
// Sample static dropdowns (replace with API data if needed)
const stones = [
  { id: 1, name: "Amethyst", code: "A", abbr: "AM." },
  { id: 2, name: "Blue Sapphire", code: "B", abbr: "BS." },
];
...
const handleImageUpload = async (file) => {
  ...
  const handleAutoGenerate = () => {
    ...
    const stoneCode =
      stones.find((s) => s.id === parseInt(form.stone_id))?.code || "";
    const shapeCode =
      shapes.find((s) => s.id === parseInt(form.shape_id))?.code || "";

    const serial = "001"; // You can replace this by calling backend to get next serial
    const sku = `${yearCode}${stoneCode}${shapeCode}-${serial}`;

    const stoneAbbr =
      stones.find((s) => s.id === parseInt(form.stone_id))?.abbr || "";
    const shapeAbbr =
      shapes.find((s) => s.id === parseInt(form.shape_id))?.abbr || "";
